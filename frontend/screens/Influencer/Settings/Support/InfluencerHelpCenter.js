...
const FAQItem = ({ question, answer, active, index, setIndex }) => (
  <Pressable
    ...
    onPress={() => {
      active ? setIndex("") : setIndex(index);
    }}
  >
...
const InfluencerHelpCenter = ({ route, navigation }) => {
  ...
  const questions = [
    {
      ...
      answer:
        "Influmart is a platform that connects influencers with brands. Brands can post campaigns and influencers apply for them.",
    },
    {
  ...
  return (
    <ScrollView style={styles.helpcenter}>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Raise Ticket</Text>
      </View>
      <View style={styles.faqsContainer}>
        <TouchableOpacity style={styles.button} onPress={()=>navigation.navigate("RaiseTicket")}>
